...
const events: AwardEvent[] = [
  {
    ...
    date: "Jan 3 • Addis Ababa",
    image: "/images/TeachingAndLearning/workshop.jpg",
    caption: "Honoring the first awakened gems who sparked ENQU SET NIQU SET.",
    nominees: [
      {
        name: "Selamawit Bekele",
        note: "Built a shelter initiative for girls after surviving displacement.",
      },
      {
        name: "Hanan Mohammed",
        note: "Uses community radio to surface unheard women’s stories.",
      },
      {
        name: "Lensa Adugna",
        note: "Mentors younger girls in digital literacy on weekends.",
      },
    ],
    winners: [
      {
  ...
  },
  {
    ...
    caption:
      "A lakeside circle honoring quiet contributors in care and education.",
    nominees: [
      {
        name: "Winta Hailemariam",
        note: "Started neighborhood care circles for new mothers.",
      },
      {
        name: "Rediet Kassa",
        note: "Teaches weekend literacy to girls who left school early.",
      },
      {
        name: "Meaza Tadesse",
        note: "Drives a mobile library van across rural villages.",
      },
    ],
    winners: [
      {
        ...
        description: "Built neighborhood care circles for new mothers.",
      },
      {
        name: "Meaza Tadesse",
        category: "Education",
        description: "Created mobile reading rooms for girls.",
      },
    ],
  },
  {
    ...
    caption:
      "Storytelling night centering survivors who became symbols of courage.",
    nominees: [
      {
        name: "Hirut Endale",
        note: "Coaches survivors to turn pain into purposeful speaking.",
      },
      {
        name: "Abigiya Gebru",
        note: "Runs a hotline that connects women to safe counsel.",
      },
      {
        name: "Mahlet Lemma",
        note: "Paints public murals that reclaim space for women.",
      },
    ],
    winners: [
      {
        name: "Abigiya Gebru",
        category: "Voice",
        description: "Opened a hotline for women seeking safe counsel.",
      },
      {
        name: "Mahlet Lemma",
        category: "Art & Healing",
        description: "Used mural art to reclaim public space for women.",
      },
    ],
  },
  {
    ...
    caption:
      "Recognizing women who lead without title, lifting villages forward.",
    nominees: [
      {
        name: "Arsema Yohannes",
        note: "Built seed banks that women manage together.",
      },
      {
        name: "Saba Fikru",
        note: "Organizes maternal health caravans to remote areas.",
      },
      {
        name: "Sofia Dawit",
        note: "Guides girls into vocational training pathways.",
      },
    ],
    winners: [
      {
        ...
        description: "Organized women-led seed banks to secure harvests.",
      },
      {
        name: "Saba Fikru",
        category: "Health",
        description: "Ran traveling clinics for maternal care.",
      },
    ],
  },
  {
    ...
    nominees: [
      {
        name: "Blain Tarekegn",
        note: "Grew cooperatives that keep craft income with women artisans.",
      },
      {
        name: "Mahiyech Moges",
        note: "Counsels youth on self-worth and confidence.",
      },
      {
        name: "Rahel Wolde",
        note: "Runs mentorship circles that feed into leadership pipelines.",
      },
    ],
    winners: [
      {
        name: "Blain Tarekegn",
        category: "Enterprise",
        description: "Built women-run cooperatives for sustainable crafts.",
      },
      {
        name: "Rahel Wolde",
        category: "Youth Mentorship",
        description: "Mentored girls into leadership pipelines.",
      },
    ],
  },
  {
    ...
    caption:
      "Highlighting allies and collaborators who open doors for awakened gems.",
    nominees: [
      {
        name: "Abeba Habte",
        note: "Funds cross-city travel so honorees can share stories.",
      },
      {
        name: "Sosina Girma",
        note: "Hosts safe lodgings for visiting honorees.",
      },
      {
        name: "Feven Alemu",
        note: "Builds volunteer crews for every gathering.",
      },
    ],
    winners: [
      {
        name: "Abeba Habte",
        category: "Allyship",
        description: "Funded cross-city travel for honorees to share stories.",
      },
      {
        name: "Feven Alemu",
        category: "Logistics",
        description:
          "Built a volunteer network that keeps events human and smooth.",
      },
    ],
  },
  {
    ...
    caption:
      "Setting the stage for the podcast era—stories that travel further.",
    nominees: [
      {
        name: "Tsion Solomon",
        note: "Preps honorees for podcast storytelling.",
      },
      {
        name: "Lily Tsegaye",
        note: "Piloted digital nomination flows in Amharic.",
      },
      {
        name: "Aida Kassahun",
        note: "Coaches girls to archive their own stories.",
      },
    ],
    winners: [
      {
        name: "Tsion Solomon",
        category: "Podcast Pioneer",
        description: "Hosts pre-launch dialogues to archive voices.",
      },
      {
        name: "Lily Tsegaye",
        category: "Innovation",
        description: "Piloted digital nomination flows in Amharic.",
      },
    ],
  },
];
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      meshes.forEach((mesh, idx) => {
        ...
        const { floatSpeed, baseY } = mesh.userData as {
          floatSpeed: number;
          baseY: number;
        };
...
export default function AwardsShowcase() {
  ...
  useEffect(() => {
    const handleWheel = (event: WheelEvent) => {
      ...
      if ((scrollingUp && canLeaveUp) || (scrollingDown && canLeaveDown))
        return;
        ...
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            const eventIdx = Number(
              entry.target.getAttribute("data-event") || 0
            );
            if (person && note) {
                ...
                <div className="flex items-center justify-between gap-4">
                  <div>
                    <div className="text-xs uppercase tracking-[0.2em] text-accent mb-2">
                      Award Session #{index + 1}
                    </div>
                    <h3 className="text-2xl lg:text-3xl font-light text-text-primary">
                      {event.title}
                    </h3>
                  </div>
                    ...
                    <ul className="space-y-2 text-text-secondary">
                      {event.nominees.map((nominee, nomineeIdx) => (
                        <li
                          ...
                          data-event={index}
                          ref={(el) =>
                            (nomineeRefs.current[index * 10 + nomineeIdx] = el)
                          }
                    ...
                    <ul className="space-y-3 text-text-secondary">
                      {event.winners.map((winner) => (
                        <li key={winner.name}>
                          ...
                          <div className="text-xs uppercase tracking-[0.16em] text-accent mb-1">
                            {winner.category}
                          </div>
